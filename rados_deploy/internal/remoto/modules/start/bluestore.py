...
def update_config(nodes, ceph_deploypath, osd_op_threads, osd_pool_size, silent):
...
def start_rados_bluestore(reservation_str, mountpoint_path, osd_op_threads, osd_pool_size, silent, retries):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(reservation)) as executor:
        ...
        if not (update_config(ceph_nodes, ceph_deploypath, osd_op_threads, osd_pool_size, silent) and restart_monitors(monitors, silent)):
            return False
        if not silent:
