...
def start_managers(managers, ceph_deploypath, silent):
    ...
    cmd = '{} --overwrite-conf mgr create {}'.format(ceph_deploypath, ' '.join(x.name for x in managers))
...
def stop_manager(manager, connection, silent):
    ...
    return code == 0
    # executors = [Executor('ssh {} "sudo systemctl stop ceph-mgr.target"'.format(x.name), **get_subprocess_kwargs(silent)) for x in managers]
...
def restart_managers(managers, silent):
    ...
    executors = [Executor('ssh {} "sudo systemctl restart ceph-mgr.target"'.format(x.name), **get_subprocess_kwargs(silent)) for x in managers]
    Executor.run_all(executors)
