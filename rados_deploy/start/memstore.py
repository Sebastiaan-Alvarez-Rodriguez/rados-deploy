...
def memstore(reservation, key_path=None, admin_id=None, connectionwrapper=None, mountpoint_path=defaults.mountpoint_path(), osd_op_threads=defaults.osd_op_threads(), osd_pool_size=defaults.osd_pool_size(), placement_groups=None, storage_size=defaults.memstore_storage_size(), silent=False, retries=defaults.retries()):
    ...
    state_ok = _start_rados(connectionwrapper.connection, rados_module, reservation, mountpoint_path, osd_op_threads, osd_pool_size, placement_groups, storage_size, silent=silent, retries=retries)
