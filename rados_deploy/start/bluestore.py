...
def _start_rados(remote_connection, module, reservation, mountpoint_path, osd_op_threads, osd_pool_size, placement_groups, use_client_cache, silent=False, retries=5):
    remote_module = remote_connection.import_module(module)
    return remote_module.start_rados_bluestore(str(reservation), mountpoint_path, osd_op_threads, osd_pool_size, placement_groups, use_client_cache, silent, retries)
...
def bluestore(reservation, key_path=None, admin_id=None, connectionwrapper=None, mountpoint_path=defaults.mountpoint_path(), osd_op_threads=defaults.osd_op_threads(), osd_pool_size=defaults.osd_pool_size(), placement_groups=None, use_client_cache=True, device_path=None, silent=False, retries=defaults.retries()):
    ...
    state_ok = _start_rados(connectionwrapper.connection, rados_module, reservation, mountpoint_path, osd_op_threads, osd_pool_size, placement_groups, use_client_cache, silent=silent, retries=retries)
