...
def deploy(parsers, args):
    if args.subcommand == 'memstore':
        ...
        return memstore(reservation, args.key_path, args.admin_id, mountpoint_path=args.mountpoint, osd_op_threads=args.osd_op_threads, osd_pool_size=args.osd_pool_size, placement_groups=args.placement_groups, storage_size=args.storage_size, silent=args.silent, retries=args.retries)[0] if reservation else False
    elif args.subcommand == 'bluestore':
        from rados_deploy.start import bluestore
        reservation = _cli_util.read_reservation_cli()
        return bluestore(reservation, args.key_path, args.admin_id, mountpoint_path=args.mountpoint, osd_op_threads=args.osd_op_threads, osd_pool_size=args.osd_pool_size, placement_groups=args.placement_groups, device_path=args.device_path, silent=args.silent, retries=args.retries)[0] if reservation else False
