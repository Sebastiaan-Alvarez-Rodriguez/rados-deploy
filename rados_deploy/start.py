...
def _generate_module_start(silent=False):
    ...
    generation_loc = fs.join(fs.dirname(fs.abspath(__file__)), 'internal', 'remoto', 'modules', 'generated', 'start_rados.py')
    files = [
...
def start(reservation, key_path=None, admin_id=None, mountpoint_path=defaults.mountpoint_path(), silent=False, retries=defaults.retries()):
